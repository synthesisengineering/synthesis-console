...
**Cockpit Mode (v0.13+) — prep packs + the People view.**

- `## 📋 Prep packs` (`prep-packs`) indexes today's meeting-prep one-pagers. The packs themselves are ritual-generated files at `<preps_dir>/YYYY-MM-DD-HHMM-slug.md` (synthesis-daily-rituals v2.12.0): H1 title, `**When:**` + `**Who:**` lines, free-form body H2s. The plan page renders one card per pack for the plan's date (from the preps_dir listing — the H2 body is optional free markdown below the cards); each card links to `/prep/:source/:slug`.
- `/people` is a **derived view with no roster file**: a 30-day scan over plans + prep packs across active sources, in the same spirit as the Rollover view. Per person: *they owe me* (leading-bold names in `waiting` sections, newest plan with a waiting section wins), *I owe them* (open drafts in the newest plan addressed to a person — @handle or bare name; #channels are excluded), *last touch* (sent drafts, ticker targets, calendar attendance), *next meeting* (newest plan's calendar events naming them), and prep packs whose `**Who:**` names them. Identity: normalized exact match with hyphens/underscores as spaces — `@tony-hunter` merges with "Tony Hunter"; "Tony" and "Tony Hunter" deliberately do not (partial-name merging produces false positives). Nothing here is hand-maintained; correcting the view means correcting the files.

**Cockpit Mode (v0.14+) — the LEDGER view.**

- `/ledger` renders the newest catch-up ledger across active sources; `/ledger/:source/:date` renders a specific sweep. Ledger files are produced by the synthesis-catchup-ledger skill at `<ledgers_dir>/YYYY-MM-DD.md` (auto-detected as `catchup-ledgers/`). The console classifies H2 sections by heading keywords ("Do now" → actionable, "Verify before re-adding" → delegated, "Done late", "Expired", "Released") and reads per-row state from a `State` table column when present (Do-now tables mix ACTIONABLE and DECAYING). Open-state sections render expanded; closed groups (done-late / expired / released) render collapsed — credit and lessons one click away, not competing with live work.
- Rows whose text reads as recognition (kudos / thanks / congrats) get a `recognition` tag chip so the consolidated-send decay pattern is visible at a glance. The tag is presentation only — sending still happens through drafts in the daily plan, which already carry the send machinery.
- The cockpit's left sidebar shows a one-row ledger digest (date + open count + decaying count) linking to the sweep, whenever the plan's source has ledgers.
